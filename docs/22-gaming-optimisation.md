...
Use the [temperature reference](appendix/temperature-reference.md) for expected temperature ranges and action points. Use the [benchmark chapter](23-benchmarks.md) for repeatable load testing after this optimisation pass.
...
1. Confirm [driver installation](20-driver-installation.md) is complete and Device Manager has no unresolved devices.
...
15. If EXPO is enabled, record the reported memory speed and run the memory checks described in [EXPO Memory Setup](19-expo.md) and [Benchmarks](23-benchmarks.md).
...
For recurring checks, also use the maintenance templates in the [checklists appendix](appendix/checklists.md).
